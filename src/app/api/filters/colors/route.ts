...
import { listColors, addColorOption, removeColorOption } from '@/lib/RentalManagementSystem';
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    const color = body?.color;
    const result = removeColorOption(color);
    if ((result as any).error) return NextResponse.json(result, { status: 404 });
    return NextResponse.json({ success: true, color: result.color });
  } catch (err) {
    return NextResponse.json({ error: 'Invalid body' }, { status: 400 });
  }
}
